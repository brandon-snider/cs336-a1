...
class Tokenizer:
    ...
    def _encode_chunk(self, text: str) -> list[int]:
        ...
        while True:
            if len(pairs_to_pretokens) == 0:
                break

            # Find the earliest merge whose pair is used in our current representations
            pair = min(pairs_to_pretokens, key=lambda p: self.merges_dict.get(p, float("inf")))
            ...
            for pretoken in pretokens_to_update:
                ...
                self._sync_pairs_to_pretokens(pairs_to_pretokens, pretoken, old_repr, new_repr)
    ...
    def _sync_pairs_to_pretokens(self, pairs_to_pretokens, pretoken, old_repr, new_repr):
